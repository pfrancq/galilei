...
static void ReadPair(/*out*/ double* x, /*out*/ double* y, char** instr)  {
...
static void Output(T* t, char** pre, char** word, int* llx, int* lly, int* urx, int* ury)
{
...
static int ParseString(T *t, char *instr, char **pre, char **word, char **post,int *llx, int *lly, int *urx, int *ury)
{
  /* Parse QS directive. */
...
int DLLEXPORT pstotextFilter(void* instance, char* instr,char** pre, char** word, char** post,int* llx, int* lly, int* urx, int* ury)
{
  T *t = (T *)instance;
  int c;
  *word = NULL;
