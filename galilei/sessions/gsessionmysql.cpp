...
void GALILEI::GSessionMySQL::ExportDM(const char* url)
{
	...
	{
		vector[i]=0;
		sprintf(sSql,"SELECT * FROM enkwds where kwdid=%u order by kwdid",i);
		RMySQL::RQuery nombre1 (this,sSql);
		for(nombre1.Begin();nombre1.IsMore();nombre1++)
		{
			j++;
			vector[i]=1;
		}
	...
	// mettre un feedback pour que l on voit ou on en est...
	for(docs.Begin();docs.IsMore();docs++)
	{
		double wvector[maxid+1];
		for(int i=0;i<maxid+1;i++)
		{
			wvector[i]=0;
		}
		sprintf(sSql,"SELECT occurs,kwdid FROM enhtmlsbykwds where htmlid=%u",atoi(docs[0]));
		RMySQL::RQuery query(this,sSql);
...
void GALILEI::GSessionMySQL::ExportP(const char* url)
{
	...
	for(profi.Begin();profi.IsMore();profi++)
	{
		...
		sprintf(sSql,"SELECT weight,kwdid FROM ensubprofilesbykwds where subprofileid=%u ",atoi(profi[0]));
		RMySQL::RQuery query(this,sSql);
		for(query.Begin();query.IsMore();query++)
		{
			wvector[atoi(query[1])]=atof(query[0]);
		}
		sprintf(sSql,"SELECT profileid FROM subprofiles where subprofileid=%u ",atoi(profi[0]));
...
void GALILEI::GSessionMySQL::ImportG(const char* url)
{
	...
	while (!(group.Eof()))
	{
		...
		groupe++;
		gtmp=groupe;
		while(isdigit(*profile))
		{
			profile++;
		}
		(*profile)=0;
		while(isdigit(*groupe))
		{
			groupe++;
		}  	
		(*groupe)=0;
		int sub=2*(atoi(ptmp))-1;
		sprintf(sSql,"insert into subprofiles(profileid,subprofileid,groupid,langid) values(%u,%u,%u,'en')",atoi(ptmp),sub,atoi(gtmp));
		RMySQL::RQuery insert(this,sSql);
		sprintf(sSql,"delete from groups where (groupid=%u)",atoi(gtmp));
		RMySQL::RQuery delet3(this,sSql);
		sprintf(sSql,"insert into groups(groupid,langid) values(%u,'en')",atoi(gtmp));
		RMySQL::RQuery insert1(this,sSql);
	}
}
