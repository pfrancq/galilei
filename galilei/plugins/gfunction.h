...
public:
	...
	*/
	...
	/**
	* Call a function. Both pointers are untyped : it is the responsibility of
	* the caller to send the right type for the parameters and to correctly
	* interpret the corresponding response.
   *
   * In practice, a number is associated to a call in order to implement
	* multiple functions in one plug-in for efficiency (for example because all
	* functions need a same memory consuming structure).
	*
	* Suppose that the method takes a string as first parameter and returns its
	* length. This method should be written:
	* @code
   ...
   *   (*len)=str->GetLen();
	...
	* @endcode
	*
	* When multiple parameters must be passed to the function, it is necessary to
	* pass one pointer to a structure that contains them.
	*
