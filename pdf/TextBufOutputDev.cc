...
void TextPage::beginWord(GfxState *state, double x0, double y0) {
  ...
  // {X,Win}SplashOutputDev rather than TextBufOutputDev).
  if (curWord) {
    ++nest;
    return;
  }
...
TextBufOutputDev::TextBufOutputDev(TextOutputFunc func, void *stream,
...
GBool TextBufOutputDev::findText(Unicode *s, int len,
			      GBool startAtTop, GBool stopAtBottom,
			      GBool startAtLast, GBool stopAtLast,
			      GBool caseSensitive, GBool backward,
			      double *xMin, double *yMin,
...
GString *TextBufOutputDev::getText(double xMin, double yMin,
				double xMax, double yMax) {
...
#if TEXTOUT_WORD_LIST
TextWordList *TextBufOutputDev::makeWordList() {
  return text->makeWordList(physLayout);
}
#endif

TextPage *TextBufOutputDev::takeText() {
  TextPage *ret;
