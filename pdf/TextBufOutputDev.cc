...
TextBufFontInfo::TextBufFontInfo(GfxState *state) {
  double *textMat;
...
TextBufFontInfo::~TextBufFontInfo() {
}

GBool TextBufFontInfo::matches(GfxState *state) {
  double *textMat;
  double t1, t2, h;
...
TextBufWord::TextBufWord(GfxState *state, double x0, double y0, int charPosA,
		   TextBufFontInfo *fontA, double fontSizeA) {
  GfxFont *gfxFont;
  double x, y;
...
TextBufWord::~TextBufWord() {
  gfree(text);
  gfree(xRight);
}

void TextBufWord::addChar(GfxState* /*state*/, double x, double /*y*/,
...
GBool TextBufLine::yxBefore(TextBufLine *line2) {
  double dy;
...
void TextBufLine::merge(TextBufLine *line2) {
  int newLen, i;
...
GBool TextBufBlock::yxBefore(TextBufBlock *blk2) {
  double dy;
...
void TextBufBlock::mergeRight(TextBufBlock *blk2) {
...
void TextBufBlock::mergeBelow(TextBufBlock *blk2) {
  TextBufLine *line;
...
TextBufPage::TextBufPage(GBool rawOrderA) {
  rawOrder = rawOrderA;
  curWord = NULL;
  charPos = 0;
  font = NULL;
...
TextBufPage::~TextBufPage() {
  clear();
  delete fonts;
}

void TextBufPage::updateFont(GfxState *state) {
  GfxFont *gfxFont;
  double *fm;
  char *name;
  ...
  for (i = 0; i < fonts->getLength(); ++i) {
    font = (TextBufFontInfo *)fonts->get(i);
  ...
  if (!font) {
    font = new TextBufFontInfo(state);
    fonts->append(font);
  }
...
void TextBufPage::beginWord(GfxState *state, double x0, double y0) {
  // This check is needed because Type 3 characters can contain
  // text-drawing operations (when TextBufPage is being used via
  // XOutputDev rather than TextBufOutputDev).
  if (curWord) {
    ++nest;
    return;
  }

  curWord = new TextBufWord(state, x0, y0, charPos, font, fontSize);
}

void TextBufPage::addChar(GfxState *state, double x, double y,
		       double dx, double dy,
...
void TextBufPage::addWord(TextBufWord *word) {
  TextBufWord *p1, *p2;
...
void TextBufPage::coalesce(GBool physLayout) {
  TextBufWord *word0, *word1, *word2;
  TextBufLine *line0, *line1, *line2, *line3, *line4, *lineList;
  TextBufBlock *blk0, *blk1, *blk2, *blk3, *blk4, *blk5, *blk6;
  TextBufBlock *yxBlocks, *blocks, *blkStack;
  TextBufFlow *flow0, *flow1;
...
double TextBufPage::lineFit(TextBufLine *line, TextBufWord *word, double *space) {
  TextBufWord *lastWord;
...
GBool TextBufPage::lineFit2(TextBufLine *line0, TextBufLine *line1) {
  double fontSize0, fontSize1;
  double dx;
...
GBool TextBufPage::blockFit(TextBufBlock *blk, TextBufLine *line) {
  double fontSize0, fontSize1;
...
GBool TextBufPage::blockFit2(TextBufBlock *blk0, TextBufBlock *blk1) {
  double fontSize0, fontSize1;
...
GBool TextBufPage::flowFit(TextBufFlow *flow, TextBufBlock *blk) {
  double dy;
...
GBool TextBufPage::findTextBuf(Unicode *s, int len,
...
GString *TextBufPage::getTextBuf(double xMin, double yMin,
			   double xMax, double yMax) {
  GString *s;
  UnicodeMap *uMap;
  GBool isUnicode;
...
GBool TextBufPage::findCharRange(int pos, int length,
			      double *xMin, double *yMin,
			      double *xMax, double *yMax) {
  TextBufLine *line;
  TextBufWord *word;
...
void TextBufPage::dump(void *outputStream, TextBufOutputFunc outputFunc,
		    GBool physLayout) {
  UnicodeMap *uMap;
...
void TextBufPage::startPage(GfxState *state) {
  clear();
  if (state) {
...
void TextBufPage::clear() {
  ...
  deleteGList(fonts, TextBufFontInfo);
...
TextBufOutputDev::TextBufOutputDev(GBool physLayoutA,
  ...
  // set up text object
  text = new TextBufPage(rawOrderA);
}


TextBufOutputDev::~TextBufOutputDev() {
  if (text) {
    delete text;
  }
  if(RCharBuffer)
...
void TextBufOutputDev::startPage(int /*pageNum*/, GfxState *state) {
  text->startPage(state);
}

void TextBufOutputDev::endPage() {
  text->coalesce(physLayout);
  text->dump(0, outputFunc, physLayout);
}

void TextBufOutputDev::updateFont(GfxState *state) {
  text->updateFont(state);
}

void TextBufOutputDev::beginString(GfxState *state, GString* /*s*/) {
  text->beginWord(state, state->getCurX(), state->getCurY());
}

void TextBufOutputDev::endString(GfxState* /*state*/) {
  text->endWord();
}

void TextBufOutputDev::drawChar(GfxState *state, double x, double y,
			     double dx, double dy,
...
GBool TextBufOutputDev::findTextBuf(Unicode *s, int len,
			      GBool top, GBool bottom,
			      double *xMin, double *yMin,
			      double *xMax, double *yMax) {
  return text->findTextBuf(s, len, top, bottom, xMin, yMin, xMax, yMax);
}

GString *TextBufOutputDev::getTextBuf(double xMin, double yMin,
				double xMax, double yMax) {
  return text->getTextBuf(xMin, yMin, xMax, yMax);
}

GBool TextBufOutputDev::findCharRange(int pos, int length,
				   double *xMin, double *yMin,
				   double *xMax, double *yMax) {
