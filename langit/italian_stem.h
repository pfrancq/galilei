...
namespace stemming
	...
	public:
		...
		void operator()(std::basic_string<Tchar_type, Tchar_traits>& text)
			...
			italian_acutes_to_graves(text);
			hash_italian_ui(text, "aeiou‡ËÏÚ˘AEIOU¿»Ã“Ÿ");

			find_r1(text, "aeiou‡ËÏÚ˘AEIOU¿»Ã“Ÿ");
			find_r2(text, "aeiou‡ËÏÚ˘AEIOU¿»Ã“Ÿ");
			find_spanish_rv(text, "aeiou‡ËÏÚ˘AEIOU¿»Ã“Ÿ");
	...
	private:
		...
		void step_1(std::basic_string<Tchar_type, Tchar_traits>& text)
			...
			else if (delete_if_is_in_r2(text, "ist‡", 4) )
				{
				return;
				}
			else if (delete_if_is_in_r2(text, "istË", 4) )
				{
				return;
				}
			else if (delete_if_is_in_r2(text, "istÏ", 4) )
				{
				return;
				}
			...
			else if (delete_if_is_in_r2(text, "it‡", 3) )
				{
		...
		void step_2(std::basic_string<Tchar_type, Tchar_traits>& text)
			...
			else if (delete_if_is_in_rv(text, "erÚ", 3, false) )
				{
				return;
				}
			...
			else if (delete_if_is_in_rv(text, "irÚ", 3, false) )
				{
				return;
				}
		...
		void step_3a(std::basic_string<Tchar_type, Tchar_traits>& text)
			...
			if (this->m_rv <= text.length()-1 &&
				string_util::is_one_of(text[text.length()-1], "aeio‡ËÏÚAEIO¿»Ã“") )
				{
