...
namespace string_util
	...
	int strnicmp(const char* first, const char* last, size_t count)
		...
		if (count)
			...
			result = static_cast<int>(static_cast<unsigned char>(f) - static_cast<unsigned char>(l) );
			}
		return result;
		}

	///Case-insensitve comparison
	int strnicmp(const wchar_t* first, const wchar_t* last, size_t count)
		{
		assert(std::wcslen(last) == count);
		wchar_t f,l;
		int result = 0;
	...
	int stricmp(const char* dst, const char* src)
		...
		return static_cast<int>(static_cast<unsigned char>(f) - static_cast<unsigned char>(l) );
		}

	int stricmp(const wchar_t* dst, const wchar_t* src)
		{
		wchar_t f,l;
		do
			{
	...
	inline wchar_t* strchr(const wchar_t* s, wchar_t ch)
		{
